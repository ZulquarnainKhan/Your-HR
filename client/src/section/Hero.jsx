    ...
    <section id="home">
      ...
      <section className="hero_sec max-w-full flex xl:flex-row flex-col justify-center min-h-screen gap-10 max-container p-4">
        ...
        <div className={`relative xl:w-2/5 flex flex-col justify-center items-start w-full max-xl:padding-x pt-28 text-container ${isVisible ? 'fade-in' : ''} max-lg:pt-10`}>
          <p className="flex items-center text-xl font-montserrat text_grad hero_intro_text"><FaArrowRightLong className="flex mr-2 hero_intro_text" style={{ color: "lightgreen" }} /> WELCOME TO YOUR-HR</p>
          <h1 className=" font-palanquin text-4xl max-md:text-[40px] max-md:leading-[40px] font-bold hero_text max-md:text-2xl">
            <span className=" relative z-10 pr-10 hero_text  " >A CAREER</span><br />
            <span className="text_grad inline-block  ">PLATFORM </span> TO FIND JOBS & INTERNSHIPS
          </h1>
          <p className="font-montserrat hero_desc_text text-lg leading-6 mt-6 mb-14 sm:max-w-sm max-md:text-[10px]">Find and apply for 2469 remote internships in various fields and locations on Your-HR. Filter by profile, location, stipend, duration, and more to get your ideal work from home internship.</p>
      ...
      <div className="flex flex-row max-lg:flex-wrap justify-evenly mb-12">
          <img src={google} className="mb-6 mx-2  company_img" />
          <img src={amazon}  className="mb-6 mx-2 company_img"/>
          <img src={spotify} className="mb-6 mx-2 company_img" />
          <img src={linkedin} className="mb-6 mx-2 company_img" />
          <img src={slack} className="mb-6 mx-2 company_img" />
          <img src={walmart} className="mb-6 mx-2 company_img" />
      </div>
      <hr/>
    </section>
